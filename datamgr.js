  ...
  for (let i = 1; i < lines.length; i++) { // skip header
    ...
    if (cols.length >= 17) {
      ...
      if (icao && runway) {
        ...
        if (!aerodromeMap[icao].includes(runway)) aerodromeMap[icao].push(runway);
      }
    }
  }
  return aerodromeMap;
  ...
  for (let i = 1; i < lines.length; i++) { // skip header
    ...
    if (cols.length >= 17 && cols[16].trim() === icao) {
      ...
      if (!isNaN(lat) && !isNaN(lng)) {
        ...
        marker.bindTooltip(cols[0].trim(), {permanent: true, direction: 'top', className: 'runway-label', offset: [0, -12] }).openTooltip();
        markers.push(marker);
        bounds.push([lat, lng]);
      }
    }
